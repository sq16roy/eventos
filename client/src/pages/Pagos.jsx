		...
		return (
		  <div key="Payment">
			<div className="App-payment">
			  <h1 className="flex-100">Pago</h1>
			  <h4 className="flex-100">Completar todos los campos</h4>
			  <ul>
				  {JSON.parse(sessionStorage.getItem('carrito')).map((e)=> {
				  	return <li>{`Evento: ${e.nombre} Precio: ${e.precio} Tickets: ${e.cantidad}`}</li>
				  })}
			  </ul>
			  <h4>{`Total: ${total-((total*15)/100)}`}</h4>
			  <Card
				number={number}
				name={name}
				expiry={expiry}
